...
function TaskCard({
  ...
  const handleSave = () => {
    ...
    onUpdateTask?.(resolvedId, { title: editTitle, description: editDescription, priority: editPriority });
    setEditingId?.(null);
  };
  ...
  return (
    ...
    >
      {onToggle && (
        <input type="checkbox" checked={!!completed} onChange={() => onToggle(resolvedId)} />
      )}
      ...
      {isEditing ? (
        ...
        <>
          ...
          {setEditingId && <Button onClick={() => setEditingId(resolvedId)}>Edit</Button>}
          {onDelete && (
            <Button
              variant="danger"
              onClick={() => {
